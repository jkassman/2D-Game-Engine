...
void gfx_update_cursor() {
    XEvent event;
    while (XCheckMaskEvent(gfx_display,-1,&event)) {
	if (event.type==MotionNotify) {
	    current_xpos = event.xkey.x;
	    current_ypos = event.xkey.y;
	} else { 
...
{
    XEvent event;
    //Note: There was a flush and a while(1) loop here.
    while (XCheckMaskEvent(gfx_display,-1,&event)) {
	if (event.type==MotionNotify) continue;
        if(event.type==KeyPress) {
	    saved_xpos = event.xkey.x;
	    saved_ypos = event.xkey.y;
	    current_xpos = saved_xpos;
	    current_ypos = saved_ypos;
	    XPutBackEvent(gfx_display,&event);
	    return 1;
